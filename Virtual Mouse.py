...
print("Executing Script by J0hnMilt0n")
...
while True:
    ...
    if len(lmlist)!=0:
        ...
        if fingers[1] == 1 and fingers[2] == 0:     # If fore finger is up and middle finger is down
            ...
            mouse.move(screen_width - curr_x, curr_y)    # Moving the cursor
        ...
        if fingers[1] == 1 and fingers[2] == 1:     # If fore finger & middle finger both are up
            ...
            if length < 30:     # If both fingers are really close to each other
                ...
                mouse.release()
                mouse.release()
            if length > 90:     # If both fingers are really close to each other
                cv2.circle(img, (lineInfo[4], lineInfo[5]), 15, (0, 255, 0), cv2.FILLED)
                mouse.double_click()    # Perform Double Click
                mouse.release()
        if fingers[1] == 1 and fingers[0] == 1:
            mouse.wheel()
        if fingers[1] == 1 and fingers[4] == 1:
            mouse.wheel(-1)
        if fingers[2] == 1 and fingers[1] == 0 and fingers[0] == 0:
            mouse.right_click()
            mouse.release()
        if fingers[2] == 0 and fingers[1] == 0 and fingers[0] == 0:
            mouse.release()
